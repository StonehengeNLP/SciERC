...
class Inconfidence:
    ...
    def get_rel_softmax_loss(rel_scores, rel_labels, num_predicted_entities):
        ...
        loss = tf.boolean_mask(loss, rel_loss_mask)
        return loss

    def call_masked(self, inputs, num_entities):
        target = tf.argmax(inputs, axis=-1)
        inconfidence = self.get_rel_softmax_loss(inputs, target, num_entities)
        return inconfidence

    def call_default(self, inputs, num_entities=None):
        logits_shape = K.int_shape(inputs)
        num_classes = logits_shape[-1]
        argmax = K.argmax(inputs, axis=-1)
        onehot = K.one_hot(argmax, num_classes)
        inconfidence = K.categorical_crossentropy(onehot, inputs,
                                                  from_logits=True)
        return inconfidence
    ...
    def __call__(self, inputs, **kwargs):
        if not self.masked:
            inconfidence = self.call_default(inputs)
        else:
            inconfidence = self.call_masked(inputs, **kwargs)

        all_but_first_axis = tuple(range(1, K.ndim(inconfidence)))

        return K.mean(inconfidence, axis=all_but_first_axis)
...
class Perturbate:
    def __init__(self, eps,
                 ...
                 method='fgsm',
                 **kwargs):
        self.eps = eps  # tf.Variable(eps, name='eps')
        self.stop_gradient = stop_gradient
        self.supports_masking = True
        self.method = method

    # def compute_output_shape(self, input_shape):
    #     logits_shape, *input_shape = input_shape
    #     return input_shape
    ...
    #     K.set_value(self.eps, eps)

    # def get_config(self):
    #     config = {'eps': float(K.get_value(self.eps)),
    #               'stop_gradient': self.stop_gradient}
    #     base_config = super().get_config()
    #     return {**base_config, **config}

    def __call__(self, inconfidence, inputs):
        grads = K.gradients(inconfidence, inputs)  # mean?
        if self.method == 'rand':
            grads = [K.random_uniform(K.shape(grad), minval=-1, maxval=1)
                     for grad in grads]
        outputs = [x - self.eps * K.sign(grad)
                   for x, grad in zip(inputs, grads)]
        if self.stop_gradient:
            outputs = K.stop_gradient(outputs)
            outputs = list(outputs)  # stop_gradient returns map
...
class SRLModel(object):
    def __init__(self, lsgn_data, config):
        ...
        self.adv = config.get("adv", False)
        self.adv_eps = config.get("adv_eps", 0)
        self.adv_method = config.get("adv_method", "fgsm")
    ...
    def get_predictions_and_loss(self, inputs, labels):
        if not self.adv:
            namespace = self.f(inputs)
            return self.g(labels, **namespace)

        namespace = self.f(inputs)
        context_emb = namespace.pop('context_emb')

        predict_dict, loss = self.g(labels, context_emb=context_emb, **namespace)

        # return predict_dict, loss

        # loss = tf.Print(loss, [
        #     K.shape(predict_dict['rel_scores']),
        #     K.shape(namespace['candidate_mask'])
        # ], 'rel_scores & candidate_mask\n\n', summarize=1000)

        inconfidence_layer = Inconfidence(masked=True)
        perturbate_layer = Perturbate(self.adv_eps, method=self.adv_method)

        logits = predict_dict['rel_scores']
        num_entities = predict_dict['num_entities']

        inconfidence = inconfidence_layer(logits,
                                          num_entities=num_entities)
        context_emb_adv, = perturbate_layer(inconfidence, [context_emb])

        old_loss = loss
        predict_dict, loss = self.g(labels, context_emb=context_emb_adv, **namespace)

        inconfidence_new = inconfidence_layer(predict_dict['rel_scores'],
                                              num_entities=num_entities)

        # loss = tf.Print(loss, [inconfidence, inconfidence_new],
        #                 '\n\ninconfidences', summarize=100)
        return predict_dict, loss

        #
        # inconfidence = inconfidence_layer()
        # context_emb_adv = perturbate_layer(loss, context_emb)
    ...
    def g(self, labels, context_emb, text_len, text_len_mask, head_emb,
          flat_candidate_starts, flat_candidate_ends,
          candidate_starts, candidate_ends, candidate_span_ids, spans_log_mask,
          max_sentence_length, max_num_candidates_per_sentence,
          flat_candidate_mask, num_sentences, inputs_doc_id, candidate_mask):

        context_outputs = lstm_contextualize(
            context_emb,
            text_len, self.config, self.lstm_dropout)  # [num_sentences, max_sentence_length, emb]
        ...
        if self.config["relation_weight"] > 0:
            ...
            candidate_entity_scores = tf.gather(
                ...
                candidate_span_ids) + spans_log_mask  # [num_sentences, max_num_candidates]
        ...
        if self.config["coref_weight"] > 0:
            ...
            doc_ids = tf.expand_dims(inputs_doc_id, 1)  # [num_sentences, 1]
